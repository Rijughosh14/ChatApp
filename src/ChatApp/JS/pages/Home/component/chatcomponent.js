...
function Chatcomponent({ id, name, set, profile, number, notification }) {
  ...
  const Chatset = (e) => {
    ...
    if (set === 0) {
      dispatchChat({ type: 'SET_USER', payload: { user: name, loggedIn: false, id: id, set: 'group', profile_pic: profile } });
    }
    else {
      dispatchChat({ type: 'SET_USER', payload: { user: name, loggedIn: false, id: id, set: 'normal', profile_pic: profile, phone_number: number } });
    }
    setnoti(null)
  }
  return (
